...
import run_public_expanded_v8 as operational_prior
...
SCHEMA = "public-expanded-regularization-execution-v8.1"
SUMMARY_SCHEMA = "public-expanded-regularization-summary-v8"
MODULES = tuple(dict.fromkeys(("run_public_expanded_v81.py", "public_expanded_v8.py",
    "prepare_public_expanded_v8.py", "public_expanded_tracking_v8.py", "run_public_expanded_v8.py", *preparation.MODULES,
...
FAILED_ROOT = Path(__file__).resolve().parents[1] / "artifacts/public_flow/expanded_v8_20260910/execution"
FAILED_CONTRACT_SHA = "e4a21b250f780b9acb02a3ccab62d5152975b89930d4e6093966c6d3ecb54bb9"
FAILED_TRACKING_SHA = "8be1cebef0d8dcdd32156a63a446fa3bbae566173caf723c5cf776f3fe3a8052"
...
def policy():
    return {"schema": SCHEMA, "model": model.policy(), "screen": prior.screen_policy(),
        "operational_change": "only allow exact W&B SDK debug-core symlink as metadata; never follow or read its target",
        "external_debug_log_bytes": "excluded; link inode bytes counted; not scientific evidence",
...
def operational_parent():
    contract_path = FAILED_ROOT / "contract.json"
    tracking_path = FAILED_ROOT / "validation/tracking/control/tracking.json"
    operational_prior.validate_contract(contract_path, FAILED_CONTRACT_SHA)
    receipt = safe.read_json(tracking_path, FAILED_TRACKING_SHA)
    for key, value in {"execution_status": "failed", "training_exit_code": 1, "event_count": 0,
                       "cloud_synced": False, "mode": "offline", "stage": "validation"}.items():
        equal(receipt.get(key), value, "preserved failed attempt " + key)
    if os.path.lexists(FAILED_ROOT / "validation/complete.json"):
        raise ValueError("Failed attempt unexpectedly marked complete")
    return {"contract": {"path": str(contract_path), "sha256": FAILED_CONTRACT_SHA},
            "tracking_failure": {"path": str(tracking_path), "sha256": FAILED_TRACKING_SHA}}
...
def validate_contract(path, sha):
    ...
    equal(sorted(value), sorted(("schema", "created_at", "registration", "protocol", "inputs", "policy",
                       "code_sha256", "expression_decoded", "operational_parent")), "execution fields")
    ...
    equal(value["operational_parent"], operational_parent(), "failed attempt lineage")
...
def sdk_debug_link_bytes(path, output_dir):
    """Count only the permitted link inode; never stat/open/resolve its target."""
    relative = path.relative_to(Path(output_dir)).as_posix()
    pattern = (r"tracking/([^/]+)/sdk/wandb/offline-run-([0-9]{8}_[0-9]{6})-([a-f0-9]{12})"
               r"/logs/debug-core\.log")
    matched = re.fullmatch(pattern, relative)
    if matched is None or matched.group(1) not in EVENTS:
        raise ValueError("Unexpected symlink output file")
    receipt = safe.read_json(Path(output_dir) / "tracking" / matched.group(1) / "tracking.json")
    for key, expected in {"run_id": matched.group(3), "mode": "offline", "group": GROUP,
                          "stage": "validation", "cloud_synced": False}.items():
        equal(receipt.get(key), expected, "SDK debug link tracking " + key)
    if receipt.get("execution_status") not in {"running", "completed", "failed"}:
        raise ValueError("Unexpected tracking lifecycle for debug link")
    target = Path(os.readlink(path))
    expected_parent = Path.home() / ".cache/wandb/logs"
    if target.parent != expected_parent or not re.fullmatch(r"core-debug-[0-9]{8}_[0-9]{6}\.log", target.name):
        raise ValueError("Unexpected W&B debug symlink target")
    info = path.lstat()
    if not stat.S_ISLNK(info.st_mode):
        raise ValueError("W&B debug link changed type")
    return info.st_size
...
def resources(output_dir, started):
    ...
    if not stat.S_ISDIR(root_path.lstat().st_mode):
        raise ValueError("Regular output root directory required")
    pending = [root_path]
    while pending:
        ...
        with os.scandir(directory) as entries:
            for entry in entries:
                ...
                if stat.S_ISLNK(info.st_mode):
                    size += sdk_debug_link_bytes(path, root_path)
    ...
    for arm in prior.model.ARMS:
        ...
        tracker = ExpandedValidationTracker(output_dir / "tracking" / arm, arm=arm,
            config={**inputs, "diagnostic_contract_sha256": contract_sha,
                    ...
                    "entrypoint_sha256": contract["code_sha256"]["run_public_expanded_v81.py"]})
        exit_code = 1
