"""Frozen sparse-output public validation; no downloads, promotion or upload."""
...
import public_sparse_hurdle_v4 as model
import run_public_joint_target_source_v3 as prior
from public_sparse_hurdle_tracking import METRICS, SparseHurdleTracker
...
SCHEMA = "public-sparse-hurdle-execution-v4"
PRIOR_SHA = "5513a0e751e500d298c4b6e29f5b6205a29160f5444da08a2e4e5a6e2d1cfafb"
SOURCES = {"nadig_jurkat", "replogle_k562"}
MODULES = ("run_public_sparse_hurdle_v4.py", "public_sparse_hurdle_v4.py",
           "public_sparse_hurdle_tracking.py", *prior.MODULES)
...
def screen_policy():
    return {"minimum_relative_primary_improvement": 0.01,
        "primary_comparators": ["control", *model.ARMS[3:]],
...
def validate_contract(path, digest):
    ...
    if (value.get("schema") != SCHEMA or value.get("policy") != model.policy()
        ...
        raise ValueError("Frozen sparse execution policy changed")
    ...
    if set(codes) != set(MODULES) or any(sha256_file(Path(__file__).with_name(k)) != v for k, v in codes.items()):
        raise ValueError("Sparse runtime implementation changed after registration")
...
def verify_summary(summary, arm, contract_sha=None):
    if (summary.get("schema") != "public-sparse-hurdle-summary-v4"
        ...
        raise ValueError("Sparse summary policy/status mismatch")
    if contract_sha is not None and (summary.get("diagnostic_contract_sha256") != contract_sha
        ...
        raise ValueError("Sparse summary contract provenance mismatch")
...
def screen(summaries):
    if set(summaries) != set(model.ARMS):
        raise ValueError("All six sparse arms required")
    ...
    for source in sorted(SOURCES):
        ...
        conditioning = {f"primary_vs_{arm}": values[arm]["target_pooled_mse"] > 0
            ...
            for arm in ("control", *model.ARMS[3:])}
...
def run(contract_path, contract_sha, output_dir):
    if socket.gethostname().split(".")[0] not in {"cbsuvlaminck3", "cbsuvlaminck6"}:
        raise RuntimeError("Sparse validation requires an approved BioHPC compute host, never a login node")
    contract, roles = validate_contract(contract_path, contract_sha)
    if os.path.lexists(output_dir):
        raise FileExistsError("Fresh sparse suite output required")
    output_dir.mkdir(mode=0o700)
    ...
    for arm in model.ARMS:
        tracker = SparseHurdleTracker(output_dir / "tracking" / arm, arm=arm, config={**inputs,
            "diagnostic_contract_sha256": contract_sha, "split_sha256": contract["registration"]["sha256"],
            "entrypoint_sha256": contract["code_sha256"]["run_public_sparse_hurdle_v4.py"]})
        exit_code = 1
        arm_dir = output_dir / "runs" / arm
        arm_dir.parent.mkdir(exist_ok=True)
        try:
            def progress(event):
                tracker.log_sparse(event["held_source"], event["metrics"])
            ...
            if audit.get("passed") is not True:
                raise ValueError("Sparse saved-artifact replay failed")
            tracker.log_sparse("all", summary["aggregate"])
            exit_code = 0
        finally:
        ...
        if (receipt.get("execution_status") != "completed" or receipt.get("training_exit_code") != 0
            ...
            raise ValueError("Offline sparse W&B recording incomplete")
    ...
    if len({r["run_id"] for r in runs}) != 6:
        raise ValueError("Distinct sparse W&B run IDs required")
    ...
    suite = {"schema": SCHEMA, "completed": True, "completed_at": completed_at,
        ...
        "download_performed": False, "cloud_synced": False, "tracking_events": 54, "tracking_errors": 0,
