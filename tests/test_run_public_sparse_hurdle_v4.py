...
import run_public_sparse_hurdle_v4 as runner
from public_sparse_hurdle_tracking import SparseHurdleTracker
...
def summary(arm, primary=1.0, mmd=.1):
    ...
    return {"schema": "public-sparse-hurdle-summary-v4", "arm": arm,
...
@pytest.mark.parametrize("arm", ("control", *runner.model.ARMS[3:]))
...
def install_synthetic_run(registered, monkeypatch, failure=None):
    ...
    def tracker(*args, **kwargs):
        ...
        sdks.append(sdk)
        result = SparseHurdleTracker(*args, sdk=sdk, **kwargs)
        if failure == "duplicate_run_ids": result.record["run_id"] = "same_synthetic_id"
        return result
    monkeypatch.setattr(runner, "SparseHurdleTracker", tracker)
    def fit(arrays, roles, tables, arm, output_dir, provenance, progress=None):
        ...
        for i in range(7 if failure == "events" else 8):
            progress({"fold_index": i, "held_source": sorted(runner.SOURCES)[i % 2], "metrics": value["aggregate"]})
...
def test_mock_six_arm_run_has_54_events_and_identical_completion_timestamps(registered, tmp_path, monkeypatch):
    ...
    assert suite["tracking_events"] == complete["tracking_events"] == 54
    ...
    assert len(sdks) == len(suite["runs"]) == 6
    for sdk in sdks:
